...
int main(int argc, char * argv[]){
 ...
 del_I = 1.60137e-40 * nHI_pix / (del_nu_21*1.e6) *(del_s * pc *1.e6 / global_hubble);
  ...
  // OUTPUT MAPS - ONLY FOR ONE PROCESSOR.
   ...
   if (myid==0)  printf("pixel value: %e \n",all_I21[87]);
  if (myid==0){
    for (int mm=0;mm<n_maps;mm++)
      {
	  nu_p = nu2 + (2.0*mm+1.0)*del_nu_lc/2.0; // middle of frequency bin being observed
	  ...
	  for (int ii=0;ii<Dim;ii++)
	      ...
	      for (int jj=0;jj<Dim;jj++)
		{
		  ...
		  all_I21[mm+n_maps*(jj+Dim*ii)] = all_I21[mm+n_maps*(jj+Dim*ii)] * pow(nu_p/nu21,3.0)   *c_m*c_m/(2.0*k_b*nu_p*nu_p*1.0e12) - numProcs*Tcmb0;
  ...
  if (myid==0){
    sprintf(fname,"%s/deltaTb_RSD/LightconeRSD_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_ds2h.dat",argv[1],Dim,FoVdeg,del_nu_lc,nu1,nu2,del_s,dsdiv,pv,oneevent);
