 ...
 //   printf(fname);
  ...
  while(fgets(line,length,paramfile)!=NULL){
    if(line[0]!='#' && line[0]!='\n'){   
      ...
      if(strcmp(second,"=")!=0) {printf("Wrong format: %s/%s/%s - need: \"name = value\" (with spaces)\n",first,second,third); exit(1);}
	...
	if(global_seed < 1) {
      ...
      /* Cosmology */
      else if(strcmp(first,"use_camb_matterpower")==0){
  ...
  if(global_pk_flag==1) {
    ...
    printf("The output P(k) file should be for z=0 and linear.\n");
...
void set_cosmology_fromCAMB(char * paramfilename) {
  ...
  printf("Camb parameter file: %s\n",paramfilename);
  if((paramfile=fopen(paramfilename,"a"))==NULL){  
  ...
  while(fgets(line,length,paramfile)!=NULL){
    if(line[0]!='#' && line[0]!='\n'){
      ...
      else if(strcmp(first,"sigma8")==0) {global_sig8_camb=atof(third);s8f=1;}
      ...
      else if(strcmp(first,"transfer_redshift(1)")==0){
	if(atof(third)>0.001) {
	  printf("Matter power spectrum output should be for z=0! Exiting...\n");
	  exit(1);
	}
      }
  ...
  if(s8f==0) {printf("Error - Camb parameter file needs to have: \"sigma8 = value\" (corresponding to Camb matter Power Spectrum)\n");exit(1);}
