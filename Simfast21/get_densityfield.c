...
double deltaK(double k,int flag, tf_parms *tf){
  ...
  if(k>0) {
    if(flag==1) {
      ind=(int)floor((log(k)-log(matrix_power[0][0]))/dlnk_camb);
    ...
    } else variance = powerFunction(k,tf); /* power spectrum at z=0 */
    variance=sqrt(variance/2.)*factor;  //power in sqrt((Mpc/h)^3)
  } else variance=0.;
...
int main(int argc, char **argv){
  ...
  printf("%ld_%d",global_N_halo, (int)(global_L));  
  ...
  if(global_pk_flag==1){
    ...
    printf("Camb matter power spectrum file: %s\n",fname);fflush(0);
    ...
    rewind(file_power);
    matrix_power[0]=(double *) malloc((iMax_ps+1)*sizeof(double));
    matrix_power[1]=(double *) malloc((iMax_ps+1)*sizeof(double));
    iMax_ps=0;
    while(!feof(file_power)){
      elem=fscanf(file_power,"%lf  %lf\n",&matrix_power[0][iMax_ps],&matrix_power[1][iMax_ps]);  
      iMax_ps++;                                              
    }
    if(matrix_power[0][iMax_ps-1] < global_dk*((global_N_halo/2)*sqrt(3.1)+1)){
      printf("Error:k-range in matter power spectrum file not large enough!\n"); 
      printf("%f  %f\n",matrix_power[0][iMax_ps-1],global_dk*((global_N_halo/2)*sqrt(3.1)+1));
      exit(1); 
    }
    if(matrix_power[0][0] > global_dk) {
      printf("Error:k-range in matter power spectrum file not small enough!\n"); 
      printf("%f  %f\n",matrix_power[0][0],global_dk);
      exit(1); 
    }
    dlnk_camb=(log(matrix_power[0][iMax_ps-1])-log(matrix_power[0][0]))/(iMax_ps-1);
    sig8_old=global_sig8_camb;
  }else {
  ...
  printf("Constructing the box in k-space...\n");fflush(0);
